...
arrAssignment:
                VAR ARR_LP ARR_RP ASSIGN ARR_LP arraystruct ARR_RP MISC_SEMI //arr = {...};
                {
                    nodeDPrint(" -ASSIGN ARRAY- (2)\n");
                    SyntaxNode* node = makeNode(5, E_OPERATION, STRING, "=", makeNode(3, E_VALUE, VARIABLE, ((Data*)$1)->sval), $4);
                    $$ = node;
                    arrSizeTmp = 0;
                }
        |       VAR ARR_LP ARR_RP ASSIGN VAR ARR_LP ARR_RP MISC_SEMI //arr0 = arr1;
                {
                    nodeDPrint(" -ASSIGN ARRAY- (2)\n");
                    if(!(((Variable*)$3)->flags & E_ARR)){
                        yyerror("ERROR - assignment of non array var to array var");
                        exit(-1);
                    }
                    SyntaxNode* node = makeNode(5, E_OPERATION, STRING, "=", makeNode(3, E_VALUE, VARIABLE, ((Data*)$1)->sval), $3);
                    $$ = node;
                    arrSizeTmp = 0;
                };
